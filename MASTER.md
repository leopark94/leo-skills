...
### 6.1 민감정보 관리 (leo secret + Apple Passwords)

- `.env`, credentials, API 키 → 절대 커밋 금지
- **`leo secret add <name>`** → macOS Keychain에 저장 (모든 leo-* 프로젝트 공유)
- **`leo secret add <name> --service <url>`** → Apple Passwords 연동 (iCloud 디바이스 동기화)
- 훅으로 자동 탐지 + 차단 (detect-secrets.sh, prompt-guard.sh)

#### 프로젝트 시크릿 매니페스트 (`.leo-secrets.yaml`)

각 프로젝트 루트에 필수 시크릿을 선언:
...
- 세션 시작 시 자동 체크 (session-checklist 훅)
- `leo secret check` — 현재 프로젝트 누락 확인
- `leo secret sync` — 모든 leo-* 프로젝트 일괄 체크

#### Apple Passwords 연동

```bash
# generic-password (Keychain만) — API 키, DB URL 등
leo secret add OPENAI_API_KEY

# internet-password (Apple Passwords + iCloud) — 웹 서비스 인증
leo secret add GITHUB_TOKEN --service github.com
leo secret add VERCEL_TOKEN --service vercel.com
```

`--service` 옵션 사용 시 Apple Passwords 앱에서 확인 가능하고 iCloud로 동기화.
